...
3. Inspect the current specifications, relevant code and diffs, recent decisions, and recorded verification. Prefer observed repository state over stale narrative.
4. Reconcile the lifecycle as one specification update:
...
5. Treat an existing `rejected/` directory as a legacy prohibited source. Read and enforce it, but do not silently rename or move its contents.
6. Update a concise specification-lifecycle section in the applicable `AGENTS.md` only when no equivalent durable contract exists. Keep module details in specifications rather than the root instructions.
7. Verify the result against the working tree. Confirm that current requirements remain discoverable, completion claims have evidence, replacement links resolve, and prohibited approaches include a safe alternative or reopening condition.
8. Report which files were created, updated, or moved and why. State verification that was not run. Remain in the current task.
